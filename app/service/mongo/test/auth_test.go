...
import (
	"bytes"
	"encoding/json"
	"latihan2/app/model" // Pastikan model LoginRequest ada di sini
	"net/http/httptest"
	"testing"
...
// TestLogin_Endpoint (menggantikan TestLoginMongo_Integration)
// Tes ini SEKARANG menggunakan variabel global 'testApp', 'testSeededUsername',
// dan 'testSeededPassword' yang didefinisikan di alumni_test.go
func TestLogin_Endpoint(t *testing.T) {

	// --- Skenario 1: Login Berhasil ---
	t.Run("Login Berhasil", func(t *testing.T) {
		reqBody := model.LoginRequest{
			Username: testSeededUsername, // <-- Gunakan var dari alumni_test.go
			Password: testSeededPassword, // <-- Gunakan var dari alumni_test.go
		}
		...
		// Gunakan 'testApp' global, bukan 'app' lokal
	...
	t.Run("Password Salah", func(t *testing.T) {
		...
		resp, err := testApp.Test(req, -1) // <-- Gunakan 'testApp'
		assert.NoError(t, err)
